...
# What this directory was called before the product and the package were both
# renamed to Trove. Read in exactly one place -- ``app_data_migration``, which
# moves such a directory into place on startup -- and kept here so the platform
# rules below are written once and answer for both names.
LEGACY_APP_NAME = "organize_archive"


def _data_dir_named(name: str) -> Path:
...
def app_data_dir() -> Path:
    ...
    return _data_dir_named(APP_NAME)


def legacy_app_data_dir() -> Path:
    """Where this application's data lived before the rename to Trove."""
    return _data_dir_named(LEGACY_APP_NAME)
