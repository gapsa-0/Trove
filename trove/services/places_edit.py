"""Places, change side: the manual attach/create/merge/undo flows.

Split from ``services/places.py`` (now the map's reports) along the same line
as ``people_edit.py``; see that module's header for the reasoning.

``place_merge_preview`` is a read and still lives here, deliberately. It is the
merge's dry run: its whole job is to show the span the merge *would* produce,
using the same survivor choice and the same merged centre. Moving it to the
read module would put it out of sight of the code it has to agree with, which
