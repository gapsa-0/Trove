"""The `trove faces` command: detect faces locally and cluster them into people."""
...
def _stage_adaface_migration(conn: sqlite3.Connection, cfg: Config) -> int:
    ...
    print(
        "\nNow re-run `trove faces` (or the GUI pipeline) to re-extract with "
        "AdaFace, then `trove faces --recluster` to restore names and cluster."
    )
    return 0
...
def _recalibrate_fiqa(conn: sqlite3.Connection, cfg: Config) -> int:
    ...
    print("\nRun `trove faces --recluster` to rebuild people from the new tiers.")
    return 0
...
def run(args: argparse.Namespace, cfg: Config) -> int:
    ...
    if not Path(cfg.db_path).exists():
        print("No database yet. Run:  trove init  then  trove scan  then  trove enrich")
        return 1
