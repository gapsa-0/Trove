"""Command-line interface for Trove.
...
# Named explicitly, not __name__: `trove` reaches main() as an imported module but
# `python -m trove.cli` makes it "__main__", and a log line should
...
def cmd_init(args: argparse.Namespace, cfg: Config) -> int:
    ...
    if cfg.roots:
        ...
        print("No archive folders configured. Add one with: trove config --add-root PATH")
    missing = _preflight()
    if missing:
        print(
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="trove", description=__doc__)
    ...
    # Not `required=True`: that turns a bare `trove` into "error: the following
...
def main(argv: list[str] | None = None) -> int:
    ...
    logger.debug("trove %s", " ".join(argv if argv is not None else sys.argv[1:]))
    # Before load(): an install predating the rename keeps its config under the
    # old directory name, and this is what puts it where load() will find it.
