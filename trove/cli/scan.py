"""The `trove scan` command: walk the roots and index/hash media files."""
...
def _report(totals: ScanStats, interrupted: bool) -> None:
    ...
    if interrupted:
        print(
            "\n\nInterrupted; progress saved. Re-run 'trove scan' to resume "
...
def run(args: argparse.Namespace, cfg: Config) -> int:
    if not Path(cfg.db_path).exists():
        print("No database yet. Run:  trove init")
        return 1
