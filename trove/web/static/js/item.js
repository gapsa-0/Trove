...
//
// What a *video* on the stage needs -- noticing that no picture arrived, the
// re-encoded stream that answers it, the transport drawn over one, and the
// panel for a file nothing here can read -- is in item-video.js. That part is
// about a format rather than about an item, and the rest of the viewer is the
// same whichever way the picture got there.
//
...
/* The viewer element itself, which holds the stage and everything laid over
   it. Exported for item-video.js, which puts its "no picture" panel beside
   the stage the same way this module does. */
...
export function stillOpen(id) { return MITEM && MITEM.id === id; }
