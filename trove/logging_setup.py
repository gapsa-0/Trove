...
_OURS = "_trove_handler"


def log_file() -> Path:
    """Path of the current log file. Also what ``trove logs --path`` prints."""
...
class _LazyRotatingFileHandler(RotatingFileHandler):
    ...
    *Lazy directory creation.* The application data directory is created by
    ...
    ``trove`` invocation materialised the data dir, and ``trove migrate-data`` then
...
def configure(level: str | None = None, *, stderr: bool = True) -> None:
    ...
    ``level`` wins over the ``TROVE_LOG_LEVEL`` environment variable, which in turn
    ...
    """
    resolved = (level or os.environ.get("TROVE_LOG_LEVEL") or DEFAULT_LEVEL).strip().upper()
