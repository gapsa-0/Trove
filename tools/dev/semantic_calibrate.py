...
DEFAULT_QUERIES = [
    "birthday cake", "a wedding", "the beach", "snowy mountains", "a dog",
    "a cat", "food on a table", "a screenshot", "a scanned document",
    "a newborn baby", "a car", "flowers", "fireworks", "a sunset",
    "people dancing", "a lake", "a barbecue", "a christmas tree",
    "a graduation ceremony", "a selfie", "swimming in the pool",
    "a football match", "snow", "an old building", "a painting",
    "people in a restaurant", "a motorcycle", "a forest", "a party at night",
    "a horse",
]
...
def load_vectors(db_path, root_id, sample, seed=20260729):
    ...
    try:
        rows = conn.execute(
            ...
            (root_id, _indexer_version())).fetchall()
    finally:
        conn.close()
...
def main():
    ...
    ap.add_argument("--sample", type=int, default=0,
                    help="score only N random embeddings (0 = all)")
    ap.add_argument("--queries", type=Path,
                    help="file of queries, one per line, replacing the defaults")
    args = ap.parse_args()
    ...
    print(f"current settings: min_similarity="
          f"{cfg.semantic_search_min_similarity}  relative_floor="
          f"{cfg.semantic_search_relative_floor}\n")

    queries = ([q.strip() for q in args.queries.read_text("utf-8").splitlines()
                if q.strip()] if args.queries else DEFAULT_QUERIES)
    ...
    print(f"Best score per query — real:   min {tops.min():.4f}  "
          f"median {np.median(tops):.4f}  max {tops.max():.4f}")
    print(f"                       absent: min {absent_tops.min():.4f}  "
          f"median {np.median(absent_tops):.4f}  max {absent_tops.max():.4f}")
    if absent_tops.max() >= tops.min():
        print("  -> the two overlap, so no absolute floor can separate them;\n"
    ...
    for floor in (0.0, 0.04, 0.05, 0.06, 0.07):
        cells = []
        for ratio in ratios:
            keeps = [(real[:, i] >= max(floor, ratio * float(real[:, i].max()))).sum()
                     for i in range(real.shape[1])]
            drops = [(absent[:, i] >= max(floor, ratio * float(absent[:, i].max()))).sum()
