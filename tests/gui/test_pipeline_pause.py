...
def _open(jm, root_id: int) -> None:
    ...
    # Nor its watch: a real one, in a daemon thread nothing here closes, which
    # outlives the test and logs from inside a later one -- and that record
    # creates the data dir a migrate-data test needs absent (logging_setup).
