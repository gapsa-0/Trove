...
def test_merge_persons_records_a_person_merges_row(tmp_path):
    ...
    link = check.execute(
        "SELECT kind FROM face_links WHERE face_a=? AND face_b=?", (row["link_a"], row["link_b"])
    ).fetchone()
...
def test_merge_pets_records_a_pet_merges_row(tmp_path):
    ...
    conn.execute(
        "INSERT INTO pets(id,name,species,cover_detection_id,detection_count,"
        "created_at) VALUES(2,NULL,'dog',2,1,?)",
        (now,),
    )
    ...
    link = check.execute(
        "SELECT kind FROM pet_links WHERE det_a=? AND det_b=?", (row["link_a"], row["link_b"])
    ).fetchone()
    assert link["kind"] == "same"
    check.close()
...
def test_unmerge_persons_reverses_the_link_and_restores_names(tmp_path):
    ...
    pins = {
        r["id"]: r["manual_person"] for r in check.execute("SELECT id, manual_person FROM faces")
    }
    assert pins[1] == "Ana"
    assert pins[2] == "Beto"
    check.close()
...
def test_unmerge_pets_reverses_the_link(tmp_path):
    ...
    assert "error" in pets_edit.unmerge_pets(str(db_path), merge_id)
...
def test_pets_apply_links_honours_a_different_cannot_link(tmp_path):
    ...
    _insert_detection(conn, 1, 1, "dog", np.array([1, 0], dtype="float32"), 0.9)
    _insert_detection(conn, 2, 2, "dog", np.array([1, 0], dtype="float32"), 0.9)
    _insert_detection(conn, 3, 3, "dog", np.array([1, 0], dtype="float32"), 0.9)
    now = db.now_iso()
    conn.execute("INSERT INTO pet_links(det_a,det_b,kind,created_at) VALUES(1,2,'same',?)", (now,))
    conn.execute("INSERT INTO pet_links(det_a,det_b,kind,created_at) VALUES(2,3,'same',?)", (now,))
    conn.execute(
        "INSERT INTO pet_links(det_a,det_b,kind,created_at) VALUES(1,3,'different',?)", (now,)
    )
    conn.commit()

    emb_rows = conn.execute("SELECT id FROM animal_detections ORDER BY id").fetchall()
...
def _catalog_for_detach(tmp_path):
    ...
    now = db.now_iso()
    conn.execute(
        "INSERT INTO persons(id,name,face_count,cover_face_id,created_at) VALUES(1,'Ana',2,1,?)",
        (now,),
    )
    vec = np.ones(2, dtype="float32").tobytes()
    conn.execute(
    ...
    conn.execute(
        """INSERT INTO faces(id,file_id,box_x,box_y,box_w,box_h,embedding,
                             person_id,manual_person,created_at)
           VALUES(2,2,0,0,1,1,?,1,'Ana',?)""",
        (vec, now),
    )
    conn.execute(
        "INSERT INTO person_files(person_id,file_id,person_name,created_at) VALUES(1,2,'Ana',?)",
        (now,),
    )
    conn.commit()
    conn.close()
...
def test_detach_file_from_person_clears_face_and_writes_cannot_link(tmp_path):
    ...
    link = check.execute("SELECT kind FROM face_links WHERE face_a=1 AND face_b=2").fetchone()
    assert link is not None and link["kind"] == "different"
    # the manual person_files tag for (person 1, file 2) is gone
    assert (
        check.execute("SELECT 1 FROM person_files WHERE person_id=1 AND file_id=2").fetchone()
        is None
    )
