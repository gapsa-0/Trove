...
def _archive(tmp_path, rows, located=()):
    ...
    for file_id in located:
        conn.execute(
            "INSERT INTO geo(file_id,lat,lon,geo_source) VALUES(?,-34.6,-58.4,'exif')",
            (file_id,))
    conn.commit()
    conn.close()
    return db_path
...
def _flags(db_path, **kw):
    return {i["id"]: i["indexed"]
            for i in queries.media(db_path, root_id=1, **kw)["items"]}
...
def test_only_a_current_vector_of_the_current_bytes_counts(tmp_path):
    ...
    assert _flags(db_path) == {1: True, 2: False, 3: False,
...
def test_coverage_composes_with_the_other_filters(tmp_path):
    ...
    assert queries.media(db_path, root_id=1, indexed=True,
...
def test_the_location_box_matches_the_pin_badge(tmp_path):
    ...
    db_path = _archive(tmp_path, [
        (1, "sha1", None), (2, "sha2", None), (3, "sha3", None),
    ], located=[1, 3])
    everything = queries.media(db_path, root_id=1)
    assert {i["id"]: i["has_gps"] for i in everything["items"]} == {
