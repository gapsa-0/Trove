...
def test_names_pins_flags_and_links_survive_the_re_extract(tmp_path):
    ...
    _redetect(conn, [(1, (101, 100, 60, 60)),
                     (2, (20, 21, 50, 50)),
                     (1, (300, 300, 40, 40))])
    ...
    rows = {(r["box_x"], r["box_y"]): r for r in conn.execute(
    ...
    new_ids = {r["id"]: (r["box_x"], r["box_y"]) for r in conn.execute(
        "SELECT id, box_x, box_y FROM faces")}
...
def test_links_between_otherwise_plain_faces_survive(tmp_path):
    ...
    conn.execute("UPDATE faces SET person_id=NULL, manual_person=NULL, "
                 "not_person=0, nonhuman_kind=NULL")
...
def test_a_box_that_moved_too_far_is_not_treated_as_the_same_face(tmp_path):
    ...
    moved = conn.execute(
        "SELECT manual_person FROM faces WHERE box_x=400").fetchone()
...
def test_reattach_is_idempotent(tmp_path):
    ...
    assert conn.execute("SELECT COUNT(*) FROM face_links").fetchone()[0] == 1
    assert conn.execute(
        "SELECT COUNT(*) FROM faces WHERE manual_person='Mari'").fetchone()[0] == 2
    conn.close()
