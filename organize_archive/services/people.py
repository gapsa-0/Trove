...
from ._common import _NOT_HIDDEN, _QUALITY_OK, _quality_ok, _root_clause
...
def face_person(db_path: str, person_id: int, root_id=None, limit=120, offset=0) -> dict | None:
...
def rename_person(db_path: str, person_id, name: str) -> dict:
    ...
    try:
        ...
        if old["name"] and name and old["name"] != name:
            conn.execute(
                "UPDATE faces SET manual_person=? WHERE manual_person=?", (name, old["name"])
            )
        conn.commit()
...
def detach_file_from_person(db_path: str, person_id, file_id) -> dict:
    ...
    try:
        p = conn.execute(
            "SELECT id, cover_face_id FROM persons WHERE id=?", (person_id,)
        ).fetchone()
        if not p:
        ...
        conn.execute(
            "DELETE FROM person_files WHERE person_id=? AND file_id=?", (person_id, file_id)
        )
        _sync_person_stats(conn, person_id)
    ...
    try:
        ...
        drop_face_ids = [
            ...
            for r in conn.execute(
                "SELECT id FROM faces WHERE person_id=?", (drop["id"],)
            ).fetchall()
        ]
        link = _record_link(
            conn,
        ...
        conn.commit()
        r = conn.execute(
            "SELECT id,name,face_count FROM persons WHERE id=?", (keep["id"],)
        ).fetchone()
        return {
            "ok": True,
